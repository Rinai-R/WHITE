...
另外，很多人会认为 `entersyscallblock` 会在调用会引发阻塞的系统调用时调用，实际上并不是这也，前面也说了，`syscall` 包里面只会在进入系统调用之前调用 `reentersyscall`，那么 `entersyscallblock` 到底是干什么的？我们可以注意到调用这个 `entersyscallblock` 的 `notetsleepg` 主要是由 `signal_recv` 调用，然后通过 linkname 链接到了 `os/signal` 包，目前来看主要是用于一个信号通知的机制，也就是我们所说的 `Notify` 会用到他，但是这里我也没咋（懒得）研究，有兴趣的可以自己看看吧。
